# crawlSystem 完整重构方案 · 24.4

> **2026-09-20 存量指标切片范围说明：** 按 `24.11_存量视频指标采集映射与提交校验.md` 读取原始 player/next 计数，避免 Number 精度损失；未公开/缺失数量不补零，明确评论关闭仍按 disabled=0。本文 S-20 的政策零值、A-14 的 API 估算等需要额外证据，当前切片尚未实现这些分支，不代表已完成全部采集规则保真验收。

> **2026-09-20 代理实现修订：采用 Crawlee 公共采集模块 + 自定义代理生命周期扩展 + YouTube.js fetch 适配。** 不部署 Rota/Resin，不引入独立代理网关或代理管理服务。下文 Rota 的接口和部署要求由 `24.10_Crawlee公共采集模块与代理生命周期实施方案.md` 替代；代理分组、并发预算、故障恢复目的仍保留，跨节点归属由后续平台配置管理实现。当前仅公共模块本地验证，不能视为真实 Worker 或完整采集链路已上线。

> **2026-09-20 用户确认的实施修订：Kafka 正式承接采集结果。** 当前链路改为 Worker → 独立 Results Topic → Data Ingestor 消费者组 → PostgreSQL；原有 Publication Outbox → Debezium → 独立 Publication Topic → Business 保留。两链共享现有 Kafka 集群，但 Topic、消费者组、进度及保留策略独立。本文涉及“Kafka 只用于分发”“Kafka 不影响采集主链”“Worker 必须直连 gRPC 入库”的旧约束被本次决定替代；具体确认语义、背压、权限、隔离与实施范围以 `24.9_Kafka采集结果入口与并发入库实施方案.md` 为准。没有把 Broker ACK 当作 PG RECEIVED/APPLIED。

## 保留采集核心、五执行角色、三 Clock 增量、Query 自生长闭环、业务控制台读模型与全链路追踪

**版本：24.4｜修订日期：2026-09-17｜状态：完整开发基准，待代码实现与业务／故障／容量验收。**

本文件以 **24.3 Query 闭环与策略定案版**为直接底稿，完整继承 24.3 已定案的五执行角色、Main Lane／Agent Lane、Data API 回流、三 Clock 增量、领域证明、轻量原子结算与 Query 自生长闭环。本版不重新选择采集／存储基础设施，而是把已经存在的持久业务对象正式组织为 **Business Console Read Model（业务控制台读模型）** 与 **Execution Trace Projection（执行链路追踪投影）**，使业务控制台能够从 Query、Candidate、Full／Refresh、API／Agent、Submission、RECEIVED／APPLIED、Domain Receipt、Feature／Clock、Plan Settlement 一直追到 Publication／DELIVERED，同时不把 UI 投影变成第二套权威状态机。Control／Worker／Data／Storage 四平面、一个逻辑 PostgreSQL＋Primary/Standby HA、SeaweedFS、ClickHouse，以及独立的 `publication.outbox → Debezium → Kafka → Business Consumer` 分发链继续保留。

**本版核心：保留经验证的采集算法与业务判定，重建外围执行与恢复；执行角色仍为发现（Discover）、全量（Full Crawl）、增量（Incremental）、官方 API 批处理（Data API Batch）、Agent 批处理（Agent Batch）。Query Engine 新增 `QueryTerm → QueryBinding(country + business_category) → QueryRun → Candidate/Channel → 新 Query Candidate → Query Pool` 闭环；Business Category 严格对应业务数据库固定的 19 个一级分类，英文为稳定领域值、中文为 UI 展示名，并与 Query Source Signal（例如 `AUTO_TAG`）明确分离；首次 Query Run 使用 `THIS_YEAR` 建立发现基线，成功后不保存连续“质量分”，只根据本轮产出与策略版本得到离散 `refresh_factor = WEEK | MONTH`，从而生成下一次 Query Clock：WEEK 对应下一周执行并携带 `THIS_WEEK`，MONTH 对应下一月执行并携带 `THIS_MONTH`；低价值 Query 可进入 `COOLDOWN / DORMANT`，后续新 Signal 可重新唤醒。 Business Console 的实时业务视图只读取／投影这些权威对象；历史趋势交 ClickHouse，底层资源诊断交 Prometheus／Grafana／Temporal UI。新增读模型必须可重建、带数据水位，不得参与 Plan 授权、写入判定、Clock 推进或 Publication 幂等。**
...
第 24 章提供中文＋英文完整链路和返回路径；第 25 章提供工程契约、AI 重构规则与 P-01～P-40 新验收；第 26 章冻结 Query 自发现飞轮、Country／Business Category 绑定、Query Clock、生命周期与 Q-01～Q-30 验收；**第 27 章正式冻结 Business Console Read Model、全链路 Execution Trace、数据水位、控制写边界与 BC-01～BC-30 验收。**原第 4～8 章的 146 条采集／恢复规则、69 原因码与 T/C/D 验收继续保留。旧源码中的 Finalize、BullMQ 等描述仍作为【现有基线】存在，不是要求在新系统重建旧队列。
...
**阅读顺序：**先读[第 24 章：重构链路](#chapter-24)、[第 25 章：工程与验收](#chapter-25)、[第 26 章：Query 自生长引擎](#chapter-26)和[第 27 章：业务控制台与全链路追踪](#chapter-27)；查原业务规则读 4～8；查事务／协议读 9～15；查规模、通信、HA 与分发读 19～23。
...
| Business Console | 新增可重建 Current Read Model 与 Execution Trace Projection；实时控制视图从 PG 权威事实派生，历史趋势读 ClickHouse，资源／日志／底层 Workflow 下钻到现有运维工具；Read Model 不是授权源也不是第二套业务状态机 |
...
| 基准 | 内容 |
|---|---|
| 文档版本 | 24.4；Business Console Read Model 与全链路追踪整合日期 2026-09-17 |
| 输入主文档 | `24.3_爬虫平台完整重构方案_Query闭环与策略定案版.md`；完整继承 24.3 及其 24.2/24.1/24 底稿、业务矩阵、执行链路与 Query Engine 定案 |
...
| 24.4 Business Console | 新增 Current Read Model、Execution Trace Projection、业务链路身份关联、数据水位、控制命令复核、业务／执行／运维三级下钻，以及 BC-01～BC-30 验收；不改变采集与结算权威状态机 |
...
27. [Business Console Read Model 与全链路追踪](#chapter-27)

看最新执行流程与业务控制系统先读 24～27；看业务规则读 4～8；看事务与错误处理读 9～10；看总体技术和部署读 1～3、11～15、19；看生产通信读 21～23。实施沿用 17 的阶段0～9，同时执行18、22、23、25、26、27的验收矩阵。
...
| 热结果数据面 | 独立 Kafka Results Topic + 多实例 Data Ingestor 消费者组 | 持久消息、分区并发、Consumer Group、消费重放 | 版本化结果、签名/身份、背压、幂等事务、回执、消费进度与隔离恢复；见 24.9 |
...
| Kafka／Debezium | **Kafka 用于结果入口及独立 Distribution Plane；Debezium 仍只捕获 Publication Outbox** | 结果和业务分发使用独立 Topic/Group/进度/保留策略；大 Raw 仍走对象存储，普通遥测及任务调度不全部 Kafka 化 |
...
**24.4 进一步收敛 Business Console 的读取方式：**第 27 章定义 `Current Read Model` 与 `Execution Trace Projection`。频道列表、首页实时业务状态和全链路详情优先从这些可重建投影读取；需要做控制动作时必须回到权威 Store 重新核对授权／版本。投影延迟必须显示 `updated_at / source_watermark`，不能因为 UI 读模型暂时落后就修改业务事实。历史趋势继续走 ClickHouse，Pod／HTTP／Temporal History 等底层诊断继续下钻现有运维工具。
...
| Worker → Data Plane | **Kafka 长连接生产者 → Results Topic → Data Ingestor 消费者组**，有界签名消息 | 分区并行、排队与重放；Broker ACK 与 PG 回执分开，见 24.9 |
...
新（2026-09-20 修订）：signed bounded record -> Kafka Results Topic -> Data Ingestor consumer group
```

二进制链路不需要为了消息系统把字节再Base64膨胀。旧协议在迁移完成前继续兼容，不原地改变已确认旧字节的Hash语义。

### 14.4 热结果主路径：Worker → Kafka → Data Ingestor，SeaweedFS不插在普通结果中间

2026-09-20 修订：以下图的入口已改为 Kafka。后续 14.5～14.8 涉及 gRPC/RPC 的具体机制保留为旧设计参考；当前实施采用 24.9 的 Kafka 消费限额、签名校验、手动 offset、PG 隔离记录及回执语义，不要求同时搭建两套主入口。
...
ResultPackage: Snapshot / Delta / Repair
      |
固定少量 Kafka Producer，长期复用
      |
独立 Results Topic（按 channel_id 分区）
      |
Data Ingestor 消费者组 × N
  ├ 身份/版本/Hash
  ├ 压缩/解压上限
  ├ max_inflight_bytes
...
| 3 Data Plane与结果协议 | Kafka Results、签名有界结果、Data Ingestor 消费者组、手动 offset、字节/事务背压、Local Journal、PG APPLIED 与隔离记录 | 分区并发、重复消费、接管/过期、PG/offset 提交窗口、保留缺口和滚动恢复通过；具体首批范围见 24.9 |
...
2026-09-20 修订：Kafka 同时是阶段3结果入口与阶段8业务分发的依赖，Debezium 仍只属于分发链。Kafka 故障会影响新采集结果的投递/消费；已经进入 PG 的事务可以独立完成。Worker 需保留未获终态确认的结果并退避，不能继续承诺 Kafka 故障仅影响 Publication。单逻辑 PG、不分片的决定不变。
...
| T-99 | 生产配置与部署依赖检查 | SCOPE | 不包含RSS/WebSub服务/任务；Kafka 结果入口与 Publication 分发分别验收，CDC 仅监听 Publication Outbox；不删除仍使用的旧通道。 |
...
| 控制台当前读模型（Console Current Read Model） | channel/query/plan 等当前业务摘要、source revision、updated_at、水位 | 可从权威事实重建；不得作为授权／幂等／Clock／结算来源 |
| 执行链路追踪投影（Execution Trace Projection） | stage、parent、domain、业务对象引用、状态、reason、时间、水位 | 只做跨对象查询与下钻；各业务对象真实 ID 仍是唯一／幂等依据 |
...
## 27. Business Console Read Model 与全链路追踪【24.4 新增定案】

### 27.1 本章解决的问题

业务控制台已经不只是“看某个队列里有多少任务”，而是需要回答四类问题：

1. **数据现在能不能用**：频道当前事实是否完整、是否新鲜、哪些域缺失、为什么缺失。
2. **今天该更新的有没有更新**：About／Video／Agent Clock 到期、Refresh Plan 进度、等待 API／Agent、逾期与恢复状态。
3. **新数据从哪里来、走到了哪里**：QueryRun → Candidate → Full → Agent／API → Apply → Feature／Clock → Publication。
4. **出现问题时如何下钻**：业务状态 → 执行对象 → Temporal／Trace／Logs／Pod，而不是把所有底层日志直接塞进 Business Console。

24.4 的原则不是增加第二套状态机，而是把 24.3 已经存在的持久业务对象组织成**可查询、可重建、可显示水位的读取投影**。任何 Read Model 丢失、延迟或重建都不能改变采集、授权、幂等、Clock、Plan Settlement 或 Publication 结果。

### 27.2 Business Console 的读取架构
...
```text
          ...
          │                     │                     │
          ▼                     ▼                     ▼
 Current Business Read     Historical Analytics    Control Commands
          │                     │                     │
          ▼                     ▼                     ▼
 PG Read Models           ClickHouse             Control API
          │                                           │
          ▼                                           ▼
 Authoritative PG Facts                         PlanStore / Temporal
          │
          ├─ Channel Current / Feature / Clock
          ├─ QueryTerm / QueryBinding / QueryRun
          ├─ Candidate / Qualification
          ├─ Full / Refresh Plan / Domain Receipt
          ├─ API Request / Agent Request / Batch
          ├─ Submission / RECEIVED / APPLIED
          └─ Publication State / Delivery Receipt

 Resource / Logs / Workflow Diagnostics
              │
              └────────> Prometheus / Grafana / Tempo / Temporal UI / Headlamp
```

**三类读、一类写继续成立：**

- 当前业务状态：PG 权威事实及其可重建 Read Model。
- 历史趋势／多维统计：ClickHouse。
- 资源／延迟／日志／底层执行诊断：Prometheus／Grafana／Tempo／Temporal UI／Headlamp。
- 暂停／恢复／取消／重试等控制写：Control API 重新读取权威授权与版本后执行。

Kafka 不进入 Business Console 的实时查询链。Kafka 属于跨系统 Distribution Plane；控制台查看 Crawler 自身状态时不能等待外部 Business DB 或 Kafka 消费进度才能知道采集事实。

### 27.3 Read Model 不是第二套权威数据库
...
Authoritative Facts
      │
      ├─ 决定：授权、幂等、写入、Clock、Plan、Publication
      │
      ▼
Read Projection
      │
...
1. 根据 `channel_control_state.status = RUNNING` 决定 Worker 是否仍有写权。
2. 根据 Dashboard 聚合数决定某 Plan 已完成。
3. 根据 ClickHouse 统计延迟结果推进 Clock。
4. 根据 Trace Projection 是否存在来判定 Submission 已 APPLIED。
5. 让 Read Model 与权威表各自维护一套无法对账的状态枚举。

允许：

- 在同一个 PostgreSQL 中保存小型 Current Projection，以避免每次列表页 JOIN 多张高频表。
- 通过可靠内部投影义务异步更新聚合／追踪投影；延迟必须有水位。
- 对极少量强实时详情直接查询权威 Store，而不是为了统一接口强迫所有读都经过投影。
- Read Model 被清空后，从权威事实和保留的不可变事件／回执重建。

### 27.4 Current Read Model：业务控制台的“当前视图”

首期建议按查询对象建立少量读模型，而不是给每张 UI 卡片造一张表。名称仅为设计示意，必须映射实际 Schema。

#### 27.4.1 `channel_control_state`

```text
channel_id
current_revision

current_completeness_state
...
这里必须把**当前数据质量**与**本轮更新执行状态**分开。例如：

```text
已有数据：COMPLETE，但 freshness = DUE
本轮计划：VIDEO WAIT_API
```

不能因为新一轮正在等待 API，就把昨天已经完整可用的数据改成“数据不完整”。同样，`完整但已到期` 与 `数据本身不完整` 是两种不同业务状态。

#### 27.4.2 `query_control_state`

保留 QueryBinding 的可查询摘要：Country、19 类 Business Category、lifecycle、refresh_factor、last_success_at、next_run_at、最近 QueryRun、source_type/source_ref 摘要与当前阻塞原因。真实 QueryBinding 仍是权威身份，读模型不重新生成 Binding。

#### 27.4.3 `plan_control_state`

保存当前／近期 Plan 的列表页摘要：plan_id、channel_id、kind、required domains、Main Lane phase、API／Agent 等待、各 Domain outcome、work_sealed、Settlement、Publication Gate、开始／更新时间。详情页需要更深证据时再读取 Request／Receipt 权威记录。

#### 27.4.4 `console_dashboard_current`

首页只保存低成本、业务可解释的当前汇总，例如：

```text
managed_channels
complete_channels
incomplete_channels
complete_but_due_channels
unknown_completeness_channels

refresh_due_today
refresh_completed_today
refresh_running
refresh_waiting_dependency
refresh_overdue

discover_new_candidates_today
full_admitted_today
first_complete_channels_today

api_waiting_channels
agent_waiting_channels
blocking_channels
...
这些指标必须有明确口径和范围，且能够从权威事实重建。`refresh_completed_today` 不能等同“今天一定有新视频”；一次合法检查没有新内容也可以完成。`first_complete_channels_today` 与“今天发现的频道”不是同一 cohort，不应直接画成假转化漏斗。

### 27.5 Execution Trace Projection：把整条业务链串起来

Business Console 允许从任一频道／Query／Plan 下钻查看业务级完整链路：
...
Qualification / Admission
    ↓
Full Plan / Refresh Plan
    ├─ Main Lane
    ├─ Agent Request / Batch
    └─ Conditional Data API Request
            ↓
        Submission
            ↓
        RECEIVED
 ...
 Publication Readiness
            ↓
 publication.outbox / event_id / revision
            ↓
       Debezium / Kafka
            ↓
 Business Consumer Receipt
            ↓
        DELIVERED
```

该投影展示**业务阶段**，不是把每一个 HTTP 请求、TLS socket、renderer parse 或 Pod 生命周期都复制进业务数据库。

### 27.6 真实业务 ID 必须保留，辅助 Trace ID 不能取代身份
...
submission_id
domain_receipt_id
feature_receipt/version
publication event_id / revision
delivery receipt
```

可以额外增加 `business_trace_id`／`trace_group_id` 方便一次 UI 查询，但它只是**辅助分组键**：

- 不替代 plan_id／request_id／submission_id／event_id 的幂等唯一性。
- 不用于授权、Fence、CAS 或 Revision 判断。
- 一个长期频道会产生很多 Refresh Plan，因此不能用一个永久 channel trace 覆盖所有历史。
- 同一 Candidate 可由多个 QueryRun 发现，来源边必须保留多对一关系，不强行伪造成单父节点树。

### 27.7 Trace Item 的最小结构

设计示意：

```text
execution_trace_item
────────────────────────────────
trace_item_id
trace_group_id          # 可选辅助分组
channel_id
stage_type
stage_ref_type
stage_ref_id
parent_trace_item_id    # 仅表示展示关系，不取代真实外键
lane                    # MAIN / API / AGENT / DATA / FEATURE / PUBLICATION
 domain                  # ABOUT / VIDEO / AGENT / ...
status
reason_code
summary
started_at
...
同一真实业务对象重复投影必须幂等。Trace Projector 可以重放；重放不能制造第二个业务任务，也不能改变原对象状态。

### 27.8 Full 与 Agent 并行时的 UI 轨迹
...
```text
Full Main        ███████ WAIT_API ─────── █████ COMPLETE
Data API                    █████ APPLIED
Agent Lane       █████████████████ COMPLETE
```

UI 必须允许看到“谁在等待谁”，但不把 API Request 已完成直接涂成 Video Domain 完成；API 结果还需要回到原 Full／Video 契约合并验收。

### 27.9 更新频道（Incremental）全链路

```text
Clock DUE
   ↓
Refresh Plan(required ABOUT/VIDEO/AGENT mask)
   ↓
Main Lane
   ├─ About
   └─ Video
       ...
       └─ Due Recheck
             │
             └─ API_PENDING → Data API → APPLIED → Resume

Agent Lane
   └─ Agent Request → Batch → APPLIED
...
只有 Agent Clock 到期时，Main Lane 在 UI 上应明确显示“本轮不需要”，而不是显示成未启动／失败。

### 27.10 Submission、RECEIVED、APPLIED、DELIVERED 必须分别展示

业务控制台的提交链不允许压缩成一个“上传成功”：

```text
CAPTURED / Result Ready
        ↓
Submission Sent
        ↓
RECEIVED
...
Plan / Publication Gate
        ↓
DELIVERED
外部 Business Consumer 本地事务已经持久接受
```

ACK 丢失或 COMMIT_UNKNOWN 时，UI 优先显示“结果未知／正在核对回执”，不能直接显示失败并诱导用户重新采集。

### 27.11 三级可观测性：业务、执行、运维

#### 第一层：Business Console 默认视图
...
#### 第二层：Execution Detail

展示：Plan、Domain、Request、Batch、Submission、Receipt、Retry Budget、Reason、Revision、Watermark。

#### 第三层：Operational Diagnostics

通过深链／关联 ID 下钻：Temporal Workflow、Trace、Logs、HTTP 诊断、Pod、Rota、PG 指标。Business Console 不复制这些工具的完整页面。

### 27.12 数据水位与一致性展示

不同读取路径有不同刷新延迟，页面必须显示水位：

```text
Current State（PG）       updated_at / source_revision
Read Projection           updated_at / source_watermark
Historical Analytics      analytics_watermark
Publication               latest_event_revision / delivery_watermark
```

规则：

1. 首页“当前状态”和历史趋势的统计时间不能混成一个时间点。
2. Read Model 超过允许滞后阈值时显示“数据更新中／统计截至 …”，不偷偷回扫全库做重查询。
3. 控制动作提交前重新读取权威状态，不能因为页面显示的旧状态直接执行。
4. Read Model 投影恢复后允许追平，不要求与业务写事务同步完成后才允许采集继续。

### 27.13 控制命令：读投影，写权威命令
...
Control API
      ↓
读取 Authoritative Store
核对 version / authorization / current state
      ↓
执行合法 Command / Intent
      ↓
权威事务提交
...
请求可携带 `expected_version` 作为乐观并发保护。若页面过旧，返回明确冲突并刷新页面，而不是覆盖新状态。
...
- 纳管频道。
- 当前完整频道／不完整频道／完整但已到期／尚未判定。
- 今日应更新／已完成／运行中／等待依赖／逾期。
- 今日新 Candidate、Full 准入、首次达到完整的数据资产。
- 当前主要阻塞原因及影响频道数。

不把 Query 来源分布、所有国家地图、所有 19 类分类分布、Pod 状态全部长期堆在首页；这些进入对应 Discover／频道／运维页面。

频道管理列表至少能看：当前完整性、当前新鲜度、本轮计划、About／Video／Agent 状态、等待原因、上次成功更新、下次到期、Publication 状态。进入频道详情后再展示 Execution Trace。

### 27.15 Read Model 更新策略

首期优先使用同一个逻辑 Crawler PostgreSQL 内的小型投影，避免新增独立数据库：

- 对当前频道／Plan 的少量字段，可以在权威事务内以低成本同步更新投影，前提是不扩大锁范围和事务耗时。
- 对跨对象 Trace、Dashboard 汇总等可采用**可靠内部投影义务**异步更新；投影器失败不阻塞采集事实提交。
- 大历史趋势不回灌 PG Projection，继续进入 ClickHouse。
- 不直接对所有 crawler 表开启 Debezium CDC 来构建 Console；Publication CDC 仍只监听 `publication.outbox`。

如果投影更新与权威事实需要分事务，必须保存稳定 source identity/revision，使 projector 可重复消费并幂等追平。

### 27.16 Read Model 的性能边界

Business Console API 不允许把“灵活”变成每次页面加载都无界 JOIN：

- 列表页只读有限分页的 Current Projection。
- 详情页按 channel_id／plan_id／request_id 精确读取。
- 首页聚合读小型汇总或 ClickHouse，不在请求时扫描全部 contents／observations。
- 高基数 ID 不放 Prometheus labels；放详情查询与 Trace。
- 最近 N 条执行历史有限返回，完整历史进入分析或按游标分页。

任何缓存必须返回 `updated_at / watermark`，缓存丢失时也不能改变业务状态。

### 27.17 Console Read Projection 与 Business Distribution Projection 必须分开

这两个 Projection 容易因为都叫“业务投影”而混淆：

```text
A. Console Read Projection
Crawler 内部使用
目的：让 Business Console 快速查询当前状态与链路
权威来源：Crawler PG
目的地：Crawler PG Read Model / ClickHouse history

B. Business Distribution Projection
跨系统使用
目的：从 Crawler facts 中选择／计算业务数据库需要的数据
权威来源：冻结 source revision 的 Crawler facts
输出：versioned business payload
      ↓
publication.outbox → Debezium → Kafka → Business Consumer → Business DB
```

Console Read Projection 不能成为外部业务数据库的数据源真相；Business Distribution Projection 也不能拿 Kafka 消费延迟替代 Crawler 当前执行状态。

需要轻量计算的分发字段可以在已有受控事务边界形成；需要跨大量视频／历史的重计算应由有界 Projection Job 使用固定 source revision 计算，成功后再以短事务写 Projection Result＋`publication.outbox`。Kafka／Debezium 负责可靠传输，不负责临时拼接 Crawler 内部多表业务模型。

### 27.18 权限、审计与敏感控制

- 页面读取权限与控制权限分开；能看状态不代表能暂停／取消。
- 人工重试、取消、强制唤醒 Query、手工准入 Candidate、人工 Publication 重放等操作记录 actor、时间、expected_version、reason 和结果。
- UI 不直接暴露数据库凭据、Kafka 管理凭据、Rota 敏感信息或外部 API Key。
- 深链到运维工具仍遵守对应工具的 RBAC。

### 27.19 Read Model 故障与重建

Read Model 故障时：

1. 采集、Apply、Clock、Plan Settlement、Publication 继续由权威事实运行。
2. Business Console 显示读取水位落后或部分页面暂不可用，不把投影故障升级成频道业务失败。
3. 修复后按 source revision／不可变事件／权威当前事实重建。
4. 重建过程不能生成新的 QueryRun／Plan／API Request／Agent Request／Publication Event。
5. 对账发现投影与权威不一致时，以权威事实为准并修复投影。

### 27.20 建议的 Business Console 页面与后端映射

| 页面 | 默认读取 | 需要下钻的权威对象 |
|---|---|---|
| 总览 | `console_dashboard_current` + ClickHouse 趋势 | Channel Current、Clock、Plan、Domain Receipt |
| 频道管理 | `channel_control_state` | Channel Current、Feature／Clock、当前 Plan |
| Discover 总览 | Query／Candidate 汇总 + ClickHouse | QueryRun、Candidate、Admission |
| Query 管理 | `query_control_state` | QueryBinding、QuerySource、QueryRun |
| 候选频道 | Candidate Read Model | Candidate、Qualification、来源 QueryRun |
| 全量抓取 | `plan_control_state(kind=FULL)` | Full Plan、Domain、Request、Receipt |
| 更新频道 | `plan_control_state(kind=REFRESH)` | Refresh Plan、About／Video／Agent、Clock |
| Agent 任务 | Agent Request／Batch Projection | Agent Request、Batch Manifest、Result Receipt |
| 数据 API | API Request／Batch Projection | API Request、Quota、Result／Apply Receipt |
| 计划任务 | Clock／Plan summary | Feature Clock、PlanStore、Scheduler decision |
| 失败与重试 | Failure／Recovery Projection | 原因证据、预算、当前 Recovery Unit |
| 提交与接收 | Submission Projection | Local Journal identity、RECEIVED、APPLIED |
| 数据质量 | Current completeness + CH quality history | Domain Receipt、Field Evidence、Review Obligation |
| 发布管理 | Publication Projection | outbox event、revision、Business delivery receipt |

### 27.21 BC-01～BC-30：Business Console 与全链路验收矩阵
...
1. 先冻结每个页面的**业务口径与数据来源**，尤其完整性、新鲜度、今日应更新、已完成、逾期、Publication／DELIVERED。
2. 建最小 `channel_control_state`、`plan_control_state`、`query_control_state`；先满足列表与详情高频查询。
3. 建 Execution Trace Projection，先串 Plan／API／Agent／Submission／Receipt，再串 Discover 来源和 Publication。
4. 建 `console_dashboard_current` 与 ClickHouse 趋势查询，所有聚合带 watermark。
5. 控制写统一走 Control API＋权威 Store 版本复核。
6. 加入 Read Model 重建工具与对账任务；投影故障不阻塞核心业务。
7. 最后做运维深链和跨页面联动，不在首期复制 Grafana／Temporal UI 功能。

### 27.23 一句话定案

> **24.4 不改变 crawlSystem 的采集与结算权威架构；它把 24.3 已经持久化的 Query、Candidate、Plan、API、Agent、Submission、Receipt、Feature、Clock 与 Publication 对象正式组织为可重建 Business Console Read Model 和 Execution Trace Projection。Business Console 可以看到采集全链路，但所有授权、幂等、完成、Clock 与发布判断仍回到权威事实；Current 从 PG 读、History 从 ClickHouse 读、底层诊断下钻现有运维工具，所有投影都有水位且可重建。**
...
## 附录 K. 24.4 Business Console 与全链路追踪修订记录

### K.1 本轮新增业务要求

- Business Console 需要看到从 Discover／Query 到 Full／Refresh、API／Agent、Submission／Apply、Feature／Clock、Publication／Delivery 的完整业务链。
- 首页和频道管理首先服务“数据是否完整／新鲜、今天该更新的是否完成、为什么阻塞”，而不是把运维指标或架构示意堆在首页。
- UI 的全链路可见性不能反过来制造第二套执行状态机；必须从真实业务对象及回执派生。
- Current 状态、历史趋势、底层运维诊断分别使用 PG Read Model、ClickHouse、Prometheus/Grafana/Temporal 等合适读取面。
- 所有控制动作必须回权威 Store 核对版本与授权；Read Model 只读、可重建、带水位。

### K.2 24.4 对 24.3 的实质新增

| 位置 | 24.4 修订 |
|---|---|
| 封面／目录／确定结论 | 升为 24.4，增加 Business Console Read Model 与 Execution Trace 作为正式目标 |
| 第 12 章 | Dashboard 读取路径增加第 27 章投影／水位／控制写边界引用 |
| 第 25 章 | 最小 durable objects 增加 Console Current Read Model 与 Execution Trace Projection |
| 新第 27 章 | 当前读模型、链路身份、Trace 投影、三级下钻、数据水位、控制 API、读写边界、页面映射和 BC-01～BC-30 验收 |
| Distribution 边界 | 明确 Console Read Projection 与跨系统 Business Distribution Projection 为两套不同目的的投影；Kafka 不进入 Crawler 控制台实时查询链 |

### K.3 明确没有改变的内容

- 不改变 Full／Incremental／Data API／Agent 的采集算法、批处理规则和恢复预算。
- 不改变 Query Engine 的 Country／19 类 Business Category／WEEK-MONTH Clock／生命周期语义。
- 不改变 PostgreSQL 单逻辑库＋HA、SeaweedFS、ClickHouse、PgBouncer、Temporal、Kubernetes 的既定方向。
- 不改变 Publication Outbox 只由 Debezium CDC 到 Kafka 的边界；不启用全库 CDC。
- 不让 Read Model／Trace Projection 成为新的授权源、完成源、Clock 源或 Publication 幂等源。
